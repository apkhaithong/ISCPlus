object HelpArmst: THelpArmst
  Left = 266
  Top = 204
  Width = 863
  Height = 501
  Caption = 'Help '#3611#3619#3632#3623#3633#3605#3636#3621#3641#3585#3588#3657#3634
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = True
  Position = poScreenCenter
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel3: TPanel
    Left = 0
    Top = 0
    Width = 855
    Height = 62
    Align = alTop
    BevelInner = bvLowered
    TabOrder = 0
    object BitBtn1: TRzRapidFireButton
      Left = 357
      Top = 13
      Width = 73
      Height = 41
      Cursor = crHandPoint
      Caption = #3588#3657#3609#3627#3634
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      Glyph.Data = {
        
        4141414141414141414141414141000000000000414141414141}
      ParentFont = False
      OnClick = BitBtn1Click
    end
    object Label1: TLabel
      Left = 40
      Top = 26
      Width = 46
      Height = 16
      Caption = #3588#3657#3609#3627#3634#3605#3634#3617
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Edit1: TEdit
      Left = 223
      Top = 25
      Width = 131
      Height = 21
      Color = clWhite
      TabOrder = 0
    end
    object RadioGp1: TComboBox
      Left = 88
      Top = 25
      Width = 129
      Height = 21
      ItemHeight = 13
      ItemIndex = 0
      TabOrder = 1
      Text = #3619#3627#3633#3626#3621#3641#3585#3588#3657#3634
      Items.Strings = (
        #3619#3627#3633#3626#3621#3641#3585#3588#3657#3634
        #3594#3639#3656#3629
        #3609#3634#3617#3626#3585#3640#3621
        #3629#3635#3648#3616#3629
        #3592#3633#3591#3627#3623#3633#3604)
    end
    object BitBtn3: TBitBtn
      Left = 497
      Top = 22
      Width = 56
      Height = 25
      Cursor = crHandPoint
      Cancel = True
      Caption = #3618#3585#3648#3621#3636#3585
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ModalResult = 7
      ParentFont = False
      TabOrder = 2
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object OkBtn: TBitBtn
      Left = 439
      Top = 22
      Width = 57
      Height = 25
      Cursor = crHandPoint
      Caption = #3648#3621#3639#3629#3585
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ModalResult = 1
      ParentFont = False
      TabOrder = 3
      Layout = blGlyphTop
      NumGlyphs = 2
    end
  end
  object DBGrid1: TRzDBGrid
    Left = 0
    Top = 62
    Width = 855
    Height = 405
    Align = alClient
    DataSource = SoHlp
    FixedColor = 16766894
    TabOrder = 1
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'CUSCOD'
        Title.Caption = #3619#3627#3633#3626#3621#3641#3585#3588#3657#3634
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -13
        Title.Font.Name = 'MS Sans Serif'
        Title.Font.Style = []
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NAME1'
        Title.Caption = #3594#3639#3656#3629
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -13
        Title.Font.Name = 'MS Sans Serif'
        Title.Font.Style = []
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NAME2'
        Title.Caption = #3609#3634#3617#3626#3585#3640#3621
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -13
        Title.Font.Name = 'MS Sans Serif'
        Title.Font.Style = []
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'BIRTHDT'
        Title.Caption = #3623#3633#3609#3648#3585#3636#3604
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -13
        Title.Font.Name = 'MS Sans Serif'
        Title.Font.Style = []
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'ADDR1'
        Title.Caption = #3610#3657#3634#3609#3648#3621#3586#3607#3637#3656
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -13
        Title.Font.Name = 'MS Sans Serif'
        Title.Font.Style = []
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'ADDR2'
        Title.Caption = #3606#3609#3609
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -13
        Title.Font.Name = 'MS Sans Serif'
        Title.Font.Style = []
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'TUMB'
        Title.Caption = #3605#3635#3610#3621
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -13
        Title.Font.Name = 'MS Sans Serif'
        Title.Font.Style = []
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'AUMP'
        Title.Caption = #3629#3635#3648#3616#3629
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -13
        Title.Font.Name = 'MS Sans Serif'
        Title.Font.Style = []
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PROV'
        Title.Caption = #3592#3633#3591#3627#3623#3633#3604
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -13
        Title.Font.Name = 'MS Sans Serif'
        Title.Font.Style = []
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'ZIP'
        Title.Caption = #3652#3611#3619#3625#3603#3637#3618#3660
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -13
        Title.Font.Name = 'MS Sans Serif'
        Title.Font.Style = []
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'TELP'
        Title.Caption = #3650#3607#3619'.'
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -13
        Title.Font.Name = 'MS Sans Serif'
        Title.Font.Style = []
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'FAX'
        Title.Caption = #3649#3615#3655#3585#3595#3660
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -13
        Title.Font.Name = 'MS Sans Serif'
        Title.Font.Style = []
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DISCT'
        Title.Caption = #3626#3656#3623#3609#3621#3604
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -13
        Title.Font.Name = 'MS Sans Serif'
        Title.Font.Style = []
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'SALLEV'
        Title.Caption = #3619#3632#3604#3633#3610#3585#3634#3619#3586#3634#3618
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -13
        Title.Font.Name = 'MS Sans Serif'
        Title.Font.Style = []
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'SALLEV'
        Title.Caption = 'SERVFL'
        Visible = True
      end>
  end
  object QHlp: TFDQuery
    CachedUpdates = True
    ConnectionName = 'HI_DBMS'
    UpdateOptions.RequestLive = True
    SQL.Strings = (
      'SELECT * FROM ARMAST')
    Left = 76
    Top = 156
  end
  object SoHlp: TDataSource
    DataSet = QHlp
    OnStateChange = SoHlpStateChange
    Left = 104
    Top = 156
  end
end
