...
procedure TFPaymnt.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TFPaymnt.DataSource1StateChange(Sender: TObject);
var
  n: Word;
begin
...
procedure TFPaymnt.DBEdit7Change(Sender: TObject);
begin
...
procedure TFPaymnt.SaveBtnClick(Sender: TObject);
begin
...
procedure TFPaymnt.DelBtnClick(Sender: TObject);
begin
  sfmain.Do_Chk_DatainGL('ACCGLMST', 'REFDOCNO', FmDmic01.QArpaytrn1BILLNO.AsString);
...
procedure TFPaymnt.CancBtnClick(Sender: TObject);
begin
  FmDmic01.QArpaytrn1.cancel;
  FmDmic01.QArpaytrn_Typ.Cancel;
end;

procedure TFPaymnt.CloseBtnClick(Sender: TObject);
begin
  Close;
end;

procedure TFPaymnt.frReport1UserFunction(const Name: string; p1, p2, p3: Variant; var Val: Variant);
begin
...
procedure TFPaymnt.frxReport1GetValue(const VarName: string;
  var Value: Variant);
begin
...
procedure TFPaymnt.PrnBtnClick(Sender: TObject);
begin
  if DBEdit17.Text = '' then
    sfmain.RaiseException('กรุณาเลือกเลขที่เอกสารก่อนพิมพ์..!');

  IF cxRadioGroup1.ItemIndex=0 then
     DM_SEC.Do_Prev_Edit_FastReport(frxReport1,'fr_Billicinv.fr3','1') else
     DM_SEC.Do_Prev_Edit_FastReport(frxReport1,'fr_Billicinv2.fr3','1');
end;

procedure TFPaymnt.actEditPrnExecute(Sender: TObject);
begin
  IF cxRadioGroup1.ItemIndex=0 then
     DM_SEC.Do_Prev_Edit_FastReport(frxReport1,'fr_Billicinv.fr3','2') else
     DM_SEC.Do_Prev_Edit_FastReport(frxReport1,'fr_Billicinv2.fr3','2');
end;

procedure TFPaymnt.FormShow(Sender: TObject);
var
  N: Word;
begin
...
procedure TFPaymnt.FormCreate(Sender: TObject);
begin
...
procedure TFPaymnt.cxDBButtonEdit1PropertiesButtonClick(Sender: TObject; AButtonIndex: Integer);
begin
...
procedure TFPaymnt.cxDBGridTranBKCODEPropertiesButtonClick(Sender: TObject; AButtonIndex: Integer);
begin
  Application.Createform(TfSrcDLG, fSrcDLG);
  if fSrcDLG.ShowModalBak = Mrok then
  begin
...
procedure TFPaymnt.cxGrid3DBTableView2PAYCODEPropertiesButtonClick(Sender: TObject; AButtonIndex: Integer);
begin
  Application.Createform(TfSrcDlg, fSrcDlg);
...
procedure TFPaymnt.cxGrid3DBTableView2BANKBOOKCODPropertiesButtonClick(Sender: TObject; AButtonIndex: Integer);
begin
