object QuirySTkForm2: TQuirySTkForm2
  Left = 311
  Top = 220
  Caption = #3626#3629#3610#3606#3634#3617#3619#3606#3607#3637#3656#3617#3637#3629#3618#3641#3656#3651#3609#3626#3605#3655#3629#3585#3607#3637#3656#3626#3634#3617#3634#3619#3606#3586#3634#3618#3652#3604#3657
  ClientHeight = 0
  ClientWidth = 120
  Color = clBtnFace
  Font.Charset = THAI_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  FormStyle = fsMDIChild
  KeyPreview = True
  OldCreateOrder = True
  Position = poDefault
  Visible = True
  WindowState = wsMaximized
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object AdvPanel1: TAdvPanel
    Left = 0
    Top = 0
    Width = 120
    Height = 0
    Align = alClient
    Font.Charset = THAI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    UseDockManager = True
    Version = '2.5.8.0'
    Caption.Color = clHighlight
    Caption.ColorTo = clNone
    Caption.Font.Charset = THAI_CHARSET
    Caption.Font.Color = clWindowText
    Caption.Font.Height = -11
    Caption.Font.Name = 'Tahoma'
    Caption.Font.Style = []
    Caption.Indent = 0
    DoubleBuffered = True
    StatusBar.Font.Charset = DEFAULT_CHARSET
    StatusBar.Font.Color = clWindowText
    StatusBar.Font.Height = -11
    StatusBar.Font.Name = 'Tahoma'
    StatusBar.Font.Style = []
    Text = ''
    FullHeight = 200
    object Label1: TLabel
      Left = 408
      Top = 21
      Width = 24
      Height = 16
      Alignment = taRightJustify
      Caption = #3618#3637#3656#3627#3657#3629
      Font.Charset = THAI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label2: TLabel
      Left = 42
      Top = 45
      Width = 14
      Height = 16
      Alignment = taRightJustify
      Caption = #3619#3640#3656#3609
      Font.Charset = THAI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label6: TLabel
      Left = 192
      Top = 18
      Width = 59
      Height = 16
      Alignment = taRightJustify
      Caption = #3626#3606#3634#3609#3607#3637#3656#3648#3585#3655#3610
      Font.Charset = THAI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label3: TLabel
      Left = 14
      Top = 20
      Width = 42
      Height = 16
      Alignment = taRightJustify
      Caption = #3603'. '#3623#3633#3609#3607#3637#3656
      Font.Charset = THAI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label4: TLabel
      Left = 424
      Top = 49
      Width = 8
      Height = 16
      Alignment = taRightJustify
      Caption = #3626#3637
      Font.Charset = THAI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label5: TLabel
      Left = 227
      Top = 48
      Width = 24
      Height = 16
      Alignment = taRightJustify
      Caption = #3649#3610#3610
      Font.Charset = THAI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Bevel1: TBevel
      Left = 1
      Top = 1
      Width = 118
      Height = 88
      Align = alTop
      ExplicitWidth = 1076
    end
    object Edit_Type: TcxButtonEdit
      Left = 434
      Top = 16
      ParentFont = False
      Properties.Buttons = <
        item
          Default = True
          Kind = bkEllipsis
        end>
      Properties.OnButtonClick = Edit_TypePropertiesButtonClick
      Style.Font.Charset = THAI_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -13
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = []
      Style.LookAndFeel.NativeStyle = False
      Style.LookAndFeel.SkinName = 'LiquidSky'
      Style.IsFontAssigned = True
      StyleDisabled.LookAndFeel.NativeStyle = False
      StyleDisabled.LookAndFeel.SkinName = 'LiquidSky'
      StyleFocused.LookAndFeel.NativeStyle = False
      StyleFocused.LookAndFeel.SkinName = 'LiquidSky'
      StyleHot.LookAndFeel.NativeStyle = False
      StyleHot.LookAndFeel.SkinName = 'LiquidSky'
      TabOrder = 0
      Width = 121
    end
    object Edit_Color: TcxButtonEdit
      Left = 434
      Top = 42
      ParentFont = False
      Properties.Buttons = <
        item
          Default = True
          Kind = bkEllipsis
        end>
      Properties.OnButtonClick = Edit_ColorPropertiesButtonClick
      Style.Font.Charset = THAI_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -13
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = []
      Style.LookAndFeel.NativeStyle = False
      Style.LookAndFeel.SkinName = 'LiquidSky'
      Style.IsFontAssigned = True
      StyleDisabled.LookAndFeel.NativeStyle = False
      StyleDisabled.LookAndFeel.SkinName = 'LiquidSky'
      StyleFocused.LookAndFeel.NativeStyle = False
      StyleFocused.LookAndFeel.SkinName = 'LiquidSky'
      StyleHot.LookAndFeel.NativeStyle = False
      StyleHot.LookAndFeel.SkinName = 'LiquidSky'
      TabOrder = 1
      Width = 121
    end
    object Edit_Model: TcxButtonEdit
      Left = 59
      Top = 42
      ParentFont = False
      Properties.Buttons = <
        item
          Default = True
          Kind = bkEllipsis
        end>
      Properties.OnButtonClick = Edit_ModelPropertiesButtonClick
      Style.Font.Charset = THAI_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -13
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = []
      Style.LookAndFeel.NativeStyle = False
      Style.LookAndFeel.SkinName = 'LiquidSky'
      Style.IsFontAssigned = True
      StyleDisabled.LookAndFeel.NativeStyle = False
      StyleDisabled.LookAndFeel.SkinName = 'LiquidSky'
      StyleFocused.LookAndFeel.NativeStyle = False
      StyleFocused.LookAndFeel.SkinName = 'LiquidSky'
      StyleHot.LookAndFeel.NativeStyle = False
      StyleHot.LookAndFeel.SkinName = 'LiquidSky'
      TabOrder = 2
      Width = 121
    end
    object Edit_Baab: TcxButtonEdit
      Left = 257
      Top = 43
      ParentFont = False
      Properties.Buttons = <
        item
          Default = True
          Kind = bkEllipsis
        end>
      Properties.OnButtonClick = Edit_BaabPropertiesButtonClick
      Style.Font.Charset = THAI_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -13
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = []
      Style.LookAndFeel.NativeStyle = False
      Style.LookAndFeel.SkinName = 'LiquidSky'
      Style.IsFontAssigned = True
      StyleDisabled.LookAndFeel.NativeStyle = False
      StyleDisabled.LookAndFeel.SkinName = 'LiquidSky'
      StyleFocused.LookAndFeel.NativeStyle = False
      StyleFocused.LookAndFeel.SkinName = 'LiquidSky'
      StyleHot.LookAndFeel.NativeStyle = False
      StyleHot.LookAndFeel.SkinName = 'LiquidSky'
      TabOrder = 3
      Width = 121
    end
    object Edit_Locat: TcxButtonEdit
      Left = 257
      Top = 17
      ParentFont = False
      Properties.Buttons = <
        item
          Default = True
          Kind = bkEllipsis
        end>
      Properties.OnButtonClick = Edit_LocatPropertiesButtonClick
      Style.Font.Charset = THAI_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -13
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = []
      Style.LookAndFeel.NativeStyle = False
      Style.LookAndFeel.SkinName = 'LiquidSky'
      Style.IsFontAssigned = True
      StyleDisabled.LookAndFeel.NativeStyle = False
      StyleDisabled.LookAndFeel.SkinName = 'LiquidSky'
      StyleFocused.LookAndFeel.NativeStyle = False
      StyleFocused.LookAndFeel.SkinName = 'LiquidSky'
      StyleHot.LookAndFeel.NativeStyle = False
      StyleHot.LookAndFeel.SkinName = 'LiquidSky'
      TabOrder = 4
      Width = 121
    end
    object XFDate: TcxDateEdit
      Left = 59
      Top = 16
      ParentFont = False
      Style.Font.Charset = THAI_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -13
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = []
      Style.LookAndFeel.NativeStyle = False
      Style.LookAndFeel.SkinName = 'LiquidSky'
      Style.IsFontAssigned = True
      StyleDisabled.LookAndFeel.NativeStyle = False
      StyleDisabled.LookAndFeel.SkinName = 'LiquidSky'
      StyleFocused.LookAndFeel.NativeStyle = False
      StyleFocused.LookAndFeel.SkinName = 'LiquidSky'
      StyleHot.LookAndFeel.NativeStyle = False
      StyleHot.LookAndFeel.SkinName = 'LiquidSky'
      TabOrder = 5
      Width = 121
    end
    object cxGrid1: TcxGrid
      Left = 1
      Top = 89
      Width = 118
      Height = 379
      Align = alClient
      Font.Charset = THAI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 6
      LookAndFeel.Kind = lfOffice11
      LookAndFeel.NativeStyle = False
      LookAndFeel.SkinName = 'LiquidSky'
      object cxGrid1DBTableView1: TcxGridDBTableView
        Navigator.Buttons.CustomButtons = <>
        Navigator.Buttons.First.Visible = False
        Navigator.Buttons.PriorPage.Visible = False
        Navigator.Buttons.Prior.Visible = False
        Navigator.Buttons.Next.Visible = False
        Navigator.Buttons.NextPage.Visible = False
        Navigator.Buttons.Last.Visible = False
        Navigator.Buttons.Insert.Hint = #3649#3607#3619#3585#3649#3606#3623
        Navigator.Buttons.Append.Hint = #3648#3614#3636#3656#3617#3649#3606#3623
        Navigator.Buttons.Append.Visible = True
        Navigator.Buttons.Delete.Hint = #3621#3610#3649#3606#3623
        Navigator.Buttons.Delete.Visible = True
        Navigator.Buttons.Edit.Visible = False
        Navigator.Buttons.Post.Visible = False
        Navigator.Buttons.Cancel.Visible = False
        Navigator.Buttons.Refresh.Visible = False
        Navigator.Buttons.SaveBookmark.Visible = False
        Navigator.Buttons.GotoBookmark.Visible = False
        Navigator.Buttons.Filter.Visible = False
        DataController.DataSource = DataSource1
        DataController.Summary.DefaultGroupSummaryItems = <
          item
            Format = #39#3619#3623#3617'  '#39',##,##0.00,'#39'  '#3619#3634#3618#3585#3634#3619#39
            Kind = skCount
            Position = spFooter
          end
          item
            Format = #39#3619#3623#3617'  '#39',##,##0.00,'#39'  '#3619#3634#3618#3585#3634#3619#39
            Kind = skCount
          end
          item
            Format = '##,##0.00'
            Kind = skSum
            Position = spFooter
          end
          item
            Format = '##,##0.00'
            Kind = skSum
          end
          item
            Format = #39#3619#3623#3617#3619#3606#3607#3633#3657#3591#3627#3617#3604'  '#39',##,##0,'#39'  '#3619#3634#3618#3585#3634#3619#39
            Kind = skCount
            Position = spFooter
            Column = cxGrid1DBTableView1GCODE
          end
          item
            Format = #39#3619#3623#3617#3619#3623#3617#3619#3606#3607#3633#3657#3591#3627#3617#3604'  '#39',##,##0,'#39'  '#3619#3634#3618#3585#3634#3619#39
            Kind = skCount
            Column = cxGrid1DBTableView1GCODE
          end>
        DataController.Summary.FooterSummaryItems = <>
        DataController.Summary.SummaryGroups = <
          item
            Links = <
              item
                Column = cxGrid1DBTableView1GCODE
              end>
            SummaryItems = <
              item
                Format = '##,##0 rows fetched'
                Kind = skCount
                Position = spFooter
                Column = cxGrid1DBTableView1STRNO
              end
              item
                Format = #3619#3623#3617' : ##,##0 '#3588#3633#3609
                Kind = skCount
                Column = cxGrid1DBTableView1STRNO
              end>
          end>
        FilterRow.Visible = True
        OptionsBehavior.AlwaysShowEditor = True
        OptionsBehavior.CellHints = True
        OptionsBehavior.FocusFirstCellOnNewRecord = True
        OptionsBehavior.GoToNextCellOnEnter = True
        OptionsBehavior.IncSearch = True
        OptionsBehavior.NavigatorHints = True
        OptionsBehavior.FocusCellOnCycle = True
        OptionsCustomize.ColumnsQuickCustomization = True
        OptionsData.Appending = True
        OptionsSelection.MultiSelect = True
        OptionsSelection.CellMultiSelect = True
        OptionsSelection.InvertSelect = False
        OptionsView.FooterMultiSummaries = True
        OptionsView.GroupFooterMultiSummaries = True
        OptionsView.GroupFooters = gfVisibleWhenExpanded
        OptionsView.Indicator = True
        OptionsView.IndicatorWidth = 20
        object cxGrid1DBTableView1GCODE: TcxGridDBColumn
          Caption = #3585#3621#3640#3656#3617#3626#3636#3609#3588#3657#3634
          DataBinding.FieldName = 'GCODE'
          Visible = False
          GroupIndex = 0
          HeaderAlignmentHorz = taCenter
          Width = 157
        end
        object cxGrid1DBTableView1STAT: TcxGridDBColumn
          Caption = #3626#3606#3634#3609#3632
          DataBinding.FieldName = 'STAT'
          PropertiesClassName = 'TcxImageComboBoxProperties'
          Properties.Items = <
            item
              Description = #3619#3606#3651#3627#3617#3656
              ImageIndex = 0
              Value = 'N'
            end
            item
              Description = #3619#3606#3648#3585#3656#3634
              Value = 'O'
            end
            item
              Description = #3619#3606' Demo'
              Value = 'D'
            end
            item
              Description = #3619#3606' Display'
              Value = 'P'
            end
            item
              Description = #3619#3606#3607#3637#3656#3629#3618#3641#3656#3629#3641#3656#3595#3656#3629#3617
              Value = 'U'
            end>
          HeaderAlignmentHorz = taCenter
          Width = 100
        end
        object cxGrid1DBTableView1RECVNO: TcxGridDBColumn
          Caption = #3648#3621#3586#3607#3637#3656#3651#3610#3619#3633#3610
          DataBinding.FieldName = 'RECVNO'
          HeaderAlignmentHorz = taCenter
          Width = 102
        end
        object cxGrid1DBTableView1RECVDT: TcxGridDBColumn
          Caption = #3623#3633#3609#3607#3637#3656#3619#3633#3610
          DataBinding.FieldName = 'RECVDT'
          HeaderAlignmentHorz = taCenter
          SortIndex = 0
          SortOrder = soDescending
          Width = 85
        end
        object cxGrid1DBTableView1STRNO: TcxGridDBColumn
          Caption = #3648#3621#3586#3606#3633#3591
          DataBinding.FieldName = 'STRNO'
          HeaderAlignmentHorz = taCenter
          Width = 135
        end
        object cxGrid1DBTableView1ENGNO: TcxGridDBColumn
          Caption = #3648#3621#3586#3648#3588#3619#3639#3656#3629#3591
          DataBinding.FieldName = 'ENGNO'
          HeaderAlignmentHorz = taCenter
          Width = 116
        end
        object cxGrid1DBTableView1REGNO: TcxGridDBColumn
          Caption = #3607#3632#3648#3610#3637#3618#3609
          DataBinding.FieldName = 'REGNO'
          HeaderAlignmentHorz = taCenter
          Width = 73
        end
        object cxGrid1DBTableView1TYPE: TcxGridDBColumn
          Caption = #3618#3637#3656#3627#3657#3629
          DataBinding.FieldName = 'TYPE'
          HeaderAlignmentHorz = taCenter
          Width = 84
        end
        object cxGrid1DBTableView1MODEL: TcxGridDBColumn
          Caption = #3619#3640#3656#3609
          DataBinding.FieldName = 'MODEL'
          HeaderAlignmentHorz = taCenter
          Width = 116
        end
        object cxGrid1DBTableView1COLOR: TcxGridDBColumn
          Caption = #3626#3637
          DataBinding.FieldName = 'COLOR'
          HeaderAlignmentHorz = taCenter
          Width = 90
        end
        object cxGrid1DBTableView1CC: TcxGridDBColumn
          Caption = #3595#3637#3595#3637'.'
          DataBinding.FieldName = 'CC'
          HeaderAlignmentHorz = taCenter
        end
        object cxGrid1DBTableView1KEYNO: TcxGridDBColumn
          Caption = #3648#3621#3586#3585#3640#3597#3649#3592
          DataBinding.FieldName = 'KEYNO'
          HeaderAlignmentHorz = taCenter
        end
      end
      object cxGrid1DBCardView1: TcxGridDBCardView
        Navigator.Buttons.CustomButtons = <>
        DataController.DataSource = DataSource1
        DataController.DetailKeyFieldNames = 'STRNO'
        DataController.MasterKeyFieldNames = 'STRNO'
        DataController.Summary.DefaultGroupSummaryItems = <>
        DataController.Summary.FooterSummaryItems = <>
        DataController.Summary.SummaryGroups = <>
        OptionsCustomize.RowMoving = True
        OptionsSelection.InvertSelect = False
        OptionsView.CardIndent = 7
        Styles.StyleSheet = GridCardViewStyleSheetDevExpress
        object cxGrid1DBCardView1STRNO: TcxGridDBCardViewRow
          Caption = #3648#3621#3586#3606#3633#3591
          DataBinding.FieldName = 'STRNO'
          Position.BeginsLayer = True
        end
        object cxGrid1DBCardView1STAT: TcxGridDBCardViewRow
          Caption = #3626#3606#3634#3609#3632
          DataBinding.FieldName = 'STAT'
          Position.BeginsLayer = True
        end
        object cxGrid1DBCardView1RVLOCAT: TcxGridDBCardViewRow
          Caption = #3626#3634#3586#3634#3607#3637#3656#3619#3633#3610
          DataBinding.FieldName = 'RVLOCAT'
          Position.BeginsLayer = True
        end
        object cxGrid1DBCardView1RVCODE: TcxGridDBCardViewRow
          Caption = #3614#3609#3633#3585#3591#3634#3609#3619#3633#3610
          DataBinding.FieldName = 'RVCODE'
          Position.BeginsLayer = True
        end
        object cxGrid1DBCardView1APCODE: TcxGridDBCardViewRow
          Caption = #3619#3627#3633#3626#3648#3592#3657#3634#3627#3609#3637#3657
          DataBinding.FieldName = 'APCODE'
          Position.BeginsLayer = True
        end
        object cxGrid1DBCardView1MANUYR: TcxGridDBCardViewRow
          Caption = #3611#3637#3619#3606
          DataBinding.FieldName = 'MANUYR'
          Position.BeginsLayer = True
        end
        object cxGrid1DBCardView1ENGNO: TcxGridDBCardViewRow
          Caption = #3648#3621#3586#3648#3588#3619#3639#3656#3629#3591
          DataBinding.FieldName = 'ENGNO'
          Position.BeginsLayer = True
        end
        object cxGrid1DBCardView1REGNO: TcxGridDBCardViewRow
          Caption = #3607#3632#3648#3610#3637#3618#3609
          DataBinding.FieldName = 'REGNO'
          Position.BeginsLayer = True
        end
      end
      object cxGrid1Level1: TcxGridLevel
        GridView = cxGrid1DBTableView1
      end
    end
    object ActivateBtn: TAdvGlowButton
      Left = 557
      Top = 16
      Width = 100
      Height = 34
      Cursor = crHandPoint
      Caption = #3588#3657#3609#3627#3634#3586#3657#3629#3617#3641#3621
      NotesFont.Charset = DEFAULT_CHARSET
      NotesFont.Color = clWindowText
      NotesFont.Height = -11
      NotesFont.Name = 'Tahoma'
      NotesFont.Style = []
      Picture.Data = {
        
        00000049454E44AE426082}
      TabOrder = 7
      OnClick = ActivateBtnClick
      Appearance.BorderColor = 14727579
      Appearance.BorderColorHot = 10079963
      Appearance.BorderColorDown = 4548219
      Appearance.Color = 15653832
      Appearance.ColorTo = 16178633
      Appearance.ColorChecked = 7915518
      Appearance.ColorCheckedTo = 11918331
      Appearance.ColorDisabled = 15921906
      Appearance.ColorDisabledTo = 15921906
      Appearance.ColorDown = 7778289
      Appearance.ColorDownTo = 4296947
      Appearance.ColorHot = 15465983
      Appearance.ColorHotTo = 11332863
      Appearance.ColorMirror = 15586496
      Appearance.ColorMirrorTo = 16245200
      Appearance.ColorMirrorHot = 5888767
      Appearance.ColorMirrorHotTo = 10807807
      Appearance.ColorMirrorDown = 946929
      Appearance.ColorMirrorDownTo = 5021693
      Appearance.ColorMirrorChecked = 10480637
      Appearance.ColorMirrorCheckedTo = 5682430
      Appearance.ColorMirrorDisabled = 11974326
      Appearance.ColorMirrorDisabledTo = 15921906
      Appearance.GradientHot = ggVertical
      Appearance.GradientMirrorHot = ggVertical
      Appearance.GradientDown = ggVertical
      Appearance.GradientMirrorDown = ggVertical
      Appearance.GradientChecked = ggVertical
    end
    object CloseBtn: TAdvGlowButton
      Left = 660
      Top = 16
      Width = 100
      Height = 34
      Cursor = crHandPoint
      Caption = #3585#3621#3633#3610#3627#3609#3657#3634#3627#3621#3633#3585
      NotesFont.Charset = DEFAULT_CHARSET
      NotesFont.Color = clWindowText
      NotesFont.Height = -11
      NotesFont.Name = 'Tahoma'
      NotesFont.Style = []
      Picture.Data = {
        
        313131}
      TabOrder = 8
      OnClick = CloseBtnClick
      Appearance.BorderColor = 14727579
      Appearance.BorderColorHot = 10079963
      Appearance.BorderColorDown = 4548219
      Appearance.Color = 15653832
      Appearance.ColorTo = 16178633
      Appearance.ColorChecked = 7915518
      Appearance.ColorCheckedTo = 11918331
      Appearance.ColorDisabled = 15921906
      Appearance.ColorDisabledTo = 15921906
      Appearance.ColorDown = 7778289
      Appearance.ColorDownTo = 4296947
      Appearance.ColorHot = 15465983
      Appearance.ColorHotTo = 11332863
      Appearance.ColorMirror = 15586496
      Appearance.ColorMirrorTo = 16245200
      Appearance.ColorMirrorHot = 5888767
      Appearance.ColorMirrorHotTo = 10807807
      Appearance.ColorMirrorDown = 946929
      Appearance.ColorMirrorDownTo = 5021693
      Appearance.ColorMirrorChecked = 10480637
      Appearance.ColorMirrorCheckedTo = 5682430
      Appearance.ColorMirrorDisabled = 11974326
      Appearance.ColorMirrorDisabledTo = 15921906
      Appearance.GradientHot = ggVertical
      Appearance.GradientMirrorHot = ggVertical
      Appearance.GradientDown = ggVertical
      Appearance.GradientMirrorDown = ggVertical
      Appearance.GradientChecked = ggVertical
    end
    object cxCheckBox1: TcxCheckBox
      Left = 58
      Top = 67
      Caption = #3604#3641#3619#3634#3618#3621#3632#3648#3629#3637#3618#3604
      ParentFont = False
      Style.Font.Charset = THAI_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -12
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = []
      Style.LookAndFeel.NativeStyle = False
      Style.LookAndFeel.SkinName = 'McSkin'
      Style.IsFontAssigned = True
      StyleDisabled.LookAndFeel.NativeStyle = False
      StyleDisabled.LookAndFeel.SkinName = 'McSkin'
      StyleFocused.LookAndFeel.NativeStyle = False
      StyleFocused.LookAndFeel.SkinName = 'McSkin'
      StyleHot.LookAndFeel.NativeStyle = False
      StyleHot.LookAndFeel.SkinName = 'McSkin'
      TabOrder = 9
      OnClick = cxCheckBox1Click
    end
  end
  object QType: TFDQuery
    ConnectionName = 'HI_DBMS'
    SQL.Strings = (
      'SELECT * FROM INVTRAN')
    Left = 644
    Top = 84
    object QTypeRECVNO: TStringField
      FieldName = 'RECVNO'
      ReadOnly = True
      FixedChar = True
      Size = 12
    end
    object QTypeRECVDT: TDateField
      FieldName = 'RECVDT'
      ReadOnly = True
    end
    object QTypeGCODE: TStringField
      FieldName = 'GCODE'
      ReadOnly = True
      FixedChar = True
      Size = 3
    end
    object QTypeTYPE: TStringField
      FieldName = 'TYPE'
      ReadOnly = True
      FixedChar = True
      Size = 12
    end
    object QTypeMODEL: TStringField
      FieldName = 'MODEL'
      ReadOnly = True
      FixedChar = True
    end
    object QTypeBAAB: TStringField
      FieldName = 'BAAB'
      ReadOnly = True
      FixedChar = True
    end
    object QTypeCOLOR: TStringField
      FieldName = 'COLOR'
      ReadOnly = True
      FixedChar = True
    end
    object QTypeCC: TFloatField
      Alignment = taLeftJustify
      FieldName = 'CC'
      ReadOnly = True
    end
    object QTypeSTAT: TStringField
      FieldName = 'STAT'
      ReadOnly = True
      FixedChar = True
      Size = 1
    end
    object QTypeSTRNO: TStringField
      FieldName = 'STRNO'
      ReadOnly = True
      FixedChar = True
    end
    object QTypeENGNO: TStringField
      FieldName = 'ENGNO'
      ReadOnly = True
      FixedChar = True
    end
    object QTypeREGNO: TStringField
      FieldName = 'REGNO'
      ReadOnly = True
      FixedChar = True
      Size = 12
    end
    object QTypeKEYNO: TStringField
      FieldName = 'KEYNO'
      ReadOnly = True
      FixedChar = True
      Size = 12
    end
    object QTypeREFNO: TStringField
      FieldName = 'REFNO'
      ReadOnly = True
      FixedChar = True
      Size = 12
    end
    object QTypeMILERT: TFloatField
      Alignment = taLeftJustify
      FieldName = 'MILERT'
      ReadOnly = True
    end
    object QTypeCRLOCAT: TStringField
      FieldName = 'CRLOCAT'
      ReadOnly = True
      FixedChar = True
      Size = 5
    end
    object QTypeMOVENO: TStringField
      FieldName = 'MOVENO'
      ReadOnly = True
      FixedChar = True
      Size = 12
    end
    object QTypeMOVEDT: TDateField
      FieldName = 'MOVEDT'
      ReadOnly = True
    end
    object QTypeRVCODE: TStringField
      FieldName = 'RVCODE'
      ReadOnly = True
      FixedChar = True
      Size = 8
    end
    object QTypeRVLOCAT: TStringField
      FieldName = 'RVLOCAT'
      ReadOnly = True
      FixedChar = True
      Size = 5
    end
    object QTypeSTDCOST: TFloatField
      Alignment = taLeftJustify
      FieldName = 'STDCOST'
      ReadOnly = True
    end
    object QTypeCRCOST: TFloatField
      Alignment = taLeftJustify
      FieldName = 'CRCOST'
      ReadOnly = True
    end
    object QTypeDISCT: TFloatField
      Alignment = taLeftJustify
      FieldName = 'DISCT'
      ReadOnly = True
    end
    object QTypeNETCOST: TFloatField
      Alignment = taLeftJustify
      FieldName = 'NETCOST'
      ReadOnly = True
    end
    object QTypeCRVAT: TFloatField
      Alignment = taLeftJustify
      FieldName = 'CRVAT'
      ReadOnly = True
    end
    object QTypeTOTCOST: TFloatField
      Alignment = taLeftJustify
      FieldName = 'TOTCOST'
      ReadOnly = True
    end
    object QTypeVATRT: TFloatField
      Alignment = taLeftJustify
      FieldName = 'VATRT'
      ReadOnly = True
    end
    object QTypeNADDCOST: TFloatField
      Alignment = taLeftJustify
      FieldName = 'NADDCOST'
      ReadOnly = True
    end
    object QTypeVADDCOST: TFloatField
      Alignment = taLeftJustify
      FieldName = 'VADDCOST'
      ReadOnly = True
    end
    object QTypeTADDCOST: TFloatField
      Alignment = taLeftJustify
      FieldName = 'TADDCOST'
      ReadOnly = True
    end
    object QTypeSTDPRC: TFloatField
      Alignment = taLeftJustify
      FieldName = 'STDPRC'
      ReadOnly = True
    end
    object QTypeSDATE: TDateField
      FieldName = 'SDATE'
      ReadOnly = True
    end
    object QTypePRICE: TFloatField
      Alignment = taLeftJustify
      FieldName = 'PRICE'
      ReadOnly = True
    end
    object QTypeBONUS: TFloatField
      Alignment = taLeftJustify
      FieldName = 'BONUS'
      ReadOnly = True
    end
    object QTypeTSALE: TStringField
      FieldName = 'TSALE'
      ReadOnly = True
      FixedChar = True
      Size = 1
    end
    object QTypeCONTNO: TStringField
      FieldName = 'CONTNO'
      ReadOnly = True
      FixedChar = True
    end
    object QTypeCURSTAT: TStringField
      FieldName = 'CURSTAT'
      ReadOnly = True
      FixedChar = True
      Size = 1
    end
    object QTypeCRDTXNO: TStringField
      FieldName = 'CRDTXNO'
      ReadOnly = True
      FixedChar = True
      Size = 12
    end
    object QTypeCRDAMT: TFloatField
      Alignment = taLeftJustify
      FieldName = 'CRDAMT'
      ReadOnly = True
    end
    object QTypeRESVNO: TStringField
      FieldName = 'RESVNO'
      ReadOnly = True
      FixedChar = True
      Size = 12
    end
    object QTypeRESVDT: TDateField
      FieldName = 'RESVDT'
      ReadOnly = True
    end
    object QTypeFLAG: TStringField
      FieldName = 'FLAG'
      ReadOnly = True
      FixedChar = True
      Size = 1
    end
    object QTypeMEMO1: TMemoField
      FieldName = 'MEMO1'
      ReadOnly = True
      BlobType = ftMemo
      Size = 512
    end
    object QTypePOSTDT: TDateField
      FieldName = 'POSTDT'
      ReadOnly = True
    end
    object QTypeINPDT: TDateTimeField
      FieldName = 'INPDT'
      ReadOnly = True
    end
    object QTypeUSERID: TStringField
      FieldName = 'USERID'
      ReadOnly = True
      FixedChar = True
      Size = 8
    end
    object QTypeDORECV: TStringField
      FieldName = 'DORECV'
      ReadOnly = True
      FixedChar = True
      Size = 15
    end
    object QTypeMANUYR: TStringField
      FieldName = 'MANUYR'
      ReadOnly = True
      FixedChar = True
      Size = 4
    end
    object QTypeAPCODE: TStringField
      FieldName = 'APCODE'
      ReadOnly = True
      FixedChar = True
      Size = 8
    end
    object QTypeTAXIN: TStringField
      FieldName = 'TAXIN'
      ReadOnly = True
      FixedChar = True
      Size = 1
    end
    object QTypeTAXDT: TDateField
      FieldName = 'TAXDT'
      ReadOnly = True
    end
  end
  object DataSource1: TDataSource
    DataSet = QType
    Left = 616
    Top = 84
  end
  object cxStyleRepository1: TcxStyleRepository
    Left = 824
    Top = 32
    PixelsPerInch = 96
    object cxStyle1: TcxStyle
      AssignedValues = [svColor]
      Color = 15451300
    end
    object cxStyle2: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 12937777
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      TextColor = clWhite
    end
    object cxStyle3: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 12937777
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      TextColor = clWhite
    end
    object cxStyle4: TcxStyle
      AssignedValues = [svColor, svFont, svTextColor]
      Color = 16247513
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      TextColor = clBlack
    end
    object cxStyle5: TcxStyle
      AssignedValues = [svColor, svTextColor]
      Color = 16247513
      TextColor = clBlack
    end
    object cxStyle6: TcxStyle
      AssignedValues = [svColor, svTextColor]
      Color = 16247513
      TextColor = clBlack
    end
    object cxStyle7: TcxStyle
      AssignedValues = [svColor]
      Color = 15451300
    end
    object cxStyle8: TcxStyle
      AssignedValues = [svColor, svTextColor]
      Color = 4707838
      TextColor = clBlack
    end
    object cxStyle9: TcxStyle
      AssignedValues = [svColor, svTextColor]
      Color = 16749885
      TextColor = clWhite
    end
    object cxStyle10: TcxStyle
      AssignedValues = [svColor, svTextColor]
      Color = 12937777
      TextColor = clWhite
    end
    object GridCardViewStyleSheetDevExpress: TcxGridCardViewStyleSheet
      Caption = 'DevExpress'
      Styles.Background = cxStyle1
      Styles.Content = cxStyle4
      Styles.ContentEven = cxStyle5
      Styles.ContentOdd = cxStyle6
      Styles.IncSearch = cxStyle8
      Styles.CaptionRow = cxStyle2
      Styles.CardBorder = cxStyle3
      Styles.Inactive = cxStyle7
      Styles.RowCaption = cxStyle9
      Styles.Selection = cxStyle10
      BuiltIn = True
    end
  end
  object cxGridPopupMenu1: TcxGridPopupMenu
    Grid = cxGrid1
    PopupMenus = <>
    Left = 400
    Top = 48
  end
end
