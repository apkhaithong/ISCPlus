...
type
  TFPo_inv02 = class(TForm)
    ...
    cxGridTranPOBYNAME: TcxGridDBColumn;
...
function TFPo_inv02.MAXNo(HField, LField, Lvalue, SQLTXT: string; Dvalue: TdateTime): string;
var
...
function TFPo_inv02.ZeroLead(St: string; len: integer): string;
var
  Zero, Chr1: string;
  j, ll: integer;
begin
...
procedure TFPo_inv02.InsBtnClick(Sender: TObject);
begin
  with QPoinv1 do
  begin
    Close;
    Sql.Clear;
    Sql.Add('SELECT * FROM PO_INVOI WHERE PONO ='''' ');
    Open;
    Insert;
  end;
  QPOTYPE.Open;
...
procedure TFPo_inv02.FormCreate(Sender: TObject);
begin
...
procedure TFPo_inv02.DataSource1StateChange(Sender: TObject);
var
  n: word;
begin
  ...
  cxTextEdit7.Enabled := SaveBtn.Visible;
  ...
  if not Dm_Po.DbConfig.Active then
    Dm_Po.DbConfig.Open;
  DBEdit8.Properties.Readonly := Dm_Po.DBConfig.Fieldbyname('H_PO').asstring = 'Y';
...
procedure TFPo_inv02.DelBtnClick(Sender: TObject);
begin
  ...
  case sfmain.AdvSmoothMessageDialog5.ExecuteDialog of
    mrOk:
      begin
        DM_SEC.ChkRecent(SFMain.Xuser_ID, 'HDSTK02_4', 'DELETE', QPoinv1PONO.AsString);
...
procedure TFPo_inv02.SaveBtnClick(Sender: TObject);
begin
  if val <> 0 then
  ...
  begin
    if Datasource1.DataSet.State = dsInsert then
      Dm_sec.ChkRecent(SFMain.Xuser_ID, 'HDSTK02_4', 'INSERT', QPoinv1PONO.AsString)
    else
      Dm_sec.ChkRecent(SFMain.Xuser_ID, 'HDSTK02_4', 'EDIT', QPoinv1PONO.AsString);
    if DM_SEC.qrRecent.UpdatesPending then
      if (DM_SEC.qrRecent.ApplyUpdates = 0) then
        DM_SEC.qrRecent.CommitUpdates
      else
...
procedure TFPo_inv02.CancBtnClick(Sender: TObject);
begin
  QPoinv1.Cancel;
end;

procedure TFPo_inv02.EditBtnClick(Sender: TObject);
begin
  ...
  if fFindPO.ShowModalPO = Mrok then
  begin
    with QPoinv1 do
    begin
      Close;
    ...
    begin
      Close;
      Sql.Clear;
      Sql.Add('SELECT * FROM PO_TRANS WHERE PONO =:XPONO ');
      PARAMS[0].ASSTRING := fFindPO.KeyNo;
      OPEN;
    end;
...
procedure TFPo_inv02.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  FPo_inv02 := Nil;
  Action := Cafree;
end;

procedure TFPo_inv02.PrnbtnClick(Sender: TObject);
begin
  if DBEdit8.Text = '' then
    sfmain.RaiseException('กรุณาเลือกเลขที่เอกสารก่อนพิมพ์..!');

  DM_SEC.Do_Prev_Edit_FastReport(frxReport1, 'fr_PrnPo.fr3', '1');
end;

procedure TFPo_inv02.FormShow(Sender: TObject);
begin
  ...
  begin
    Close;
    Sql.Clear;
    Sql.Add('SELECT * FROM PO_TRANS WHERE PONO='''' ');
    Open;
  end;
end;

procedure TFPo_inv02.ClosBtnClick(Sender: TObject);
begin
  close;
end;

procedure TFPo_inv02.CalBtnClick(Sender: TObject);
begin
...
procedure TFPo_inv02.DBkbd1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  case Key of
    Vk_F9:
      PrnBtnClick(Sender);
...
procedure TFPo_inv02.CloseBtnClick(Sender: TObject);
begin
  close;
end;

procedure TFPo_inv02.cxGridTranPARTNOPropertiesButtonClick(Sender: TObject; AButtonIndex: Integer);
begin
  Application.Createform(TfFindPO, fFindPO);
  fFindPO.SC1 := XSC1;
...
procedure TFPo_inv02.DBEdit22PropertiesButtonClick(Sender: TObject; AButtonIndex: Integer);
begin
  Application.Createform(TfFindPO, fFindPO);
...
procedure TFPo_inv02.cxDBButtonEdit1PropertiesButtonClick(Sender: TObject; AButtonIndex: Integer);
begin
  Application.Createform(TfFindPO, fFindPO);
...
procedure TFPo_inv02.cxDBButtonEdit1PropertiesChange(Sender: TObject);
begin
  with Query1 do
  begin
    Close;
...
procedure TFPo_inv02.DBEdit22PropertiesChange(Sender: TObject);
begin
  cxTextEdit1.Text := SFMain.FindShowDesc('INVLOCAT', 'LOCATNM', 'LOCATCD', DBEdit22.text);
end;

procedure TFPo_inv02.cxDBButtonEdit3PropertiesButtonClick(Sender: TObject; AButtonIndex: Integer);
begin
  Application.Createform(TfFindPO, fFindPO);
...
procedure TFPo_inv02.cxDBButtonEdit2PropertiesButtonClick(Sender: TObject; AButtonIndex: Integer);
begin
  Application.Createform(TfFindPO, fFindPO);
  if fFindPO.ShowModalPOTYPE = Mrok then
  begin
    QPoinv1.FieldByName('POTYPE').Asstring := fFindPO.Keyno;
    if cxDBRadioGroup1.Itemindex = 0 then
      QPoinv1.FieldByName('Disct').AsFloat := fFindPO.qrFindDat.fieldbyname('PODISC').AsFloat;
  end;
end;

procedure TFPo_inv02.cxDBButtonEdit2PropertiesChange(Sender: TObject);
begin
  cxTextEdit3.Text := SFMain.FindShowDesc('POTYPE', 'PODESC', 'POCODE', cxDBButtonEdit2.text);
end;

procedure TFPo_inv02.cxDBButtonEdit3PropertiesChange(Sender: TObject);
begin
  with Query1 do
  begin
    close;
...
procedure TFPo_inv02.QPoTrnNewRecord(DataSet: TDataSet);
begin
...
procedure TFPo_inv02.QPoinv1AfterPost(DataSet: TDataSet);
var
  S: string;
begin
  {Apply CachedUpdate}
...
procedure TFPo_inv02.QPoinv1BeforeDelete(DataSet: TDataSet);
begin
...
procedure TFPo_inv02.QPoTrnAfterPost(DataSet: TDataSet);
var
...
begin
  ...
  while not QPoTrn.eof do
  begin
  //  Showmessage('TOTCOST = '+Floattostr(QPoTrnTOTCOST.Asfloat));
    Smtot := Smtot + Fround(QPoTrnTOTCOST.Asfloat, 2);
    QPoTrn.Next;
  end;
  QPoTrn.Enablecontrols;
...
procedure TFPo_inv02.QPoinv1APCODEValidate(Sender: TField);
begin
  with Query1 do
  begin
    Close;
...
procedure TFPo_inv02.QPoinv1AfterCancel(DataSet: TDataSet);
begin
  if QPoinv1.Active then
...
procedure TFPo_inv02.QPoinv1NewRecord(DataSet: TDataSet);
begin
  Duplicate := False;
  if not CondPay.Active then
    CondPay.Open;
...
procedure TFPo_inv02.QPoTrnBeforePost(DataSet: TDataSet);
var
  Present: TDateTime;
...
procedure TFPo_inv02.QPoinv1AfterInsert(DataSet: TDataSet);
begin
  with QPoTrn do
  begin
    Close;
...
procedure TFPo_inv02.QPoinv1BeforePost(DataSet: TDataSet);
begin
...
procedure TFPo_inv02.QPoinv1CalcFields(DataSet: TDataSet);
const
...
procedure TFPo_inv02.QPoinv1POTYPEValidate(Sender: TField);
var
...
begin
  ...
  begin
    ...
    Begin
      ...
      If Duplicate Then
        QPoinv1PONO.Asstring := Dm_po.Maxno(HH,LF,LC,Sqltxt,DV)
      Else
      Begin
...
procedure TFPo_inv02.QPoinv1CREDTMValidate(Sender: TField);
begin
  QPoinv1DUEDATE.AsDateTime := QPoinv1PODATE.Asdatetime + QPoinv1CREDTM.Asfloat;
end;

procedure TFPo_inv02.QPoTrnORDQTYValidate(Sender: TField);
begin
  ...
  begin
    ...
    begin
      ...
      Params[0].Asstring := QPoinv1POTYPE.Asstring;
      Params[1].Asstring := QPoinv1POLOCAT.Asstring;
...
procedure TFPo_inv02.QPoTrnORDCOSTValidate(Sender: TField);
begin
...
procedure TFPo_inv02.QPoTrnREDUValidate(Sender: TField);
begin
  QPoTrnNetcost.Asfloat := FRound(QPoTrnORDCOST.Asfloat * (1 - QPoTrnRedu.Asfloat / 100), 4);
  QPoTrnTOTCOST.Asfloat := QPoTrnORDQTY.Asfloat * QPoTrnNetcost.Asfloat;
end;

procedure TFPo_inv02.QPoTrnPARTNOValidate(Sender: TField);
begin
  if QPoTrn.State = Dsbrowse then
    QPoTrn.Edit;
...
procedure TFPo_inv02.QPoTrnBeforeDelete(DataSet: TDataSet);
var
  delp: string;
  ord: Integer;
begin
...
procedure TFPo_inv02.cxDBRadioGroup1Click(Sender: TObject);
begin
...
procedure TFPo_inv02.cxDBRadioGroup1PropertiesChange(Sender: TObject);
begin
  if Datasource1.State in DsEditmodes then
  begin
...
procedure TFPo_inv02.cxDBRadioGroup3PropertiesChange(Sender: TObject);
begin
  if Datasource1.State in DsEditModes then
    CalBtn.Enabled := cxDBRadioGroup3.Itemindex = 1;
end;

procedure TFPo_inv02.PoDateExit(Sender: TObject);
var
...
procedure TFPo_inv02.cxDBDateEdit1Exit(Sender: TObject);
var
  Year1, Month1, Day1, RYear: word;
begin
  if Datasource1.State in dsEditmodes then
...
procedure TFPo_inv02.cxDBDateEdit2Exit(Sender: TObject);
var
  Year1, Month1, Day1, RYear: word;
begin
  if Datasource1.State in dsEditmodes then
...
procedure TFPo_inv02.actEditPrnExecute(Sender: TObject);
begin
  DM_SEC.Do_Prev_Edit_FastReport(frxReport1, 'fr_PrnPo.fr3', '2');
end;

procedure TFPo_inv02.frReport1UserFunction(const Name: string; p1, p2, p3: Variant; var Val: Variant);
begin
...
procedure TFPo_inv02.frxReport1GetValue(const VarName: string;
  var Value: Variant);
begin
...
procedure TFPo_inv02.QPoinv1PODATEValidate(Sender: TField);
var
...
begin
  ...
  begin
    ...
    if QPoinv1UpdateStatus.Value = 'Inserted' then
    begin
      sqltxt := 'SELECT MAX(PONO) AS MAXNO FROM PO_INVOI WHERE POLOCAT=:EDIT1 AND ' +
...
procedure TFPo_inv02.QPoinv1POLOCATValidate(Sender: TField);
begin
  QPoinv1PODATEValidate(Sender);
end;

procedure TFPo_inv02.cxGridTranCustomDrawIndicatorCell(Sender: TcxGridTableView; ACanvas: TcxCanvas; AViewInfo: TcxCustomGridIndicatorItemViewInfo; var ADone: Boolean);
var
...
procedure TFPo_inv02.cxDBButtonEdit4PropertiesButtonClick(Sender: TObject; AButtonIndex: Integer);
begin
  Application.Createform(TfFindPO, fFindPO);
...
procedure TFPo_inv02.cxDBButtonEdit4PropertiesChange(Sender: TObject);
begin
  with Query1 do
  begin
    Close;
    Sql.Clear;
    Sql.Add('SELECT * FROM REELOCAT WHERE RLCODE =:APCODE');
    Params[0].Asstring := cxDBButtonEdit4.Text;
    Open;
  end;
  cxTextEdit5.Text := Query1.Fieldbyname('RLNAME').Asstring;
end;

procedure TFPo_inv02.PoHisBtnClick(Sender: TObject);
begin
  if QPoTrnPARTNO.Asstring = '' then
    sfmain.RaiseException('ไม่พบรหัสสินค้าที่อ้างอิง..!');

  Application.CreateForm(TFRecvHistory, FRecvHistory);
  FRecvHistory.cxButtonEdit1.Text := QPoTrnPARTNO.Asstring;
  FRecvHistory.ShowModal;
end;

procedure TFPo_inv02.QPoTrnPARTNOChange(Sender: TField);
begin
  with Query2 do
  begin
    Close;
...
procedure TFPo_inv02.cxDBMemo1Click(Sender: TObject);
begin
  KeyPreview := False;
end;

procedure TFPo_inv02.cxDBMemo1Exit(Sender: TObject);
begin
  KeyPreview := True;
end;

procedure TFPo_inv02.cxButtonEdit1PropertiesChange(Sender: TObject);
var
...
begin
  ...
  nPartno := Query3.fieldbyname('PARTNO').AsString;
  cxTextEdit6.Text := Query3.fieldbyname('DESC1').AsString;

  if (Query3.Fieldbyname('NETPRC').Asstring = 'N') then
  begin
    nUprice := Query3.Fieldbyname('STDBUY').AsFloat;
  end
  else
  begin
    LVPrice := 'Price1';
    nUprice := Query3.Fieldbyname(LVPrice).AsFloat;
  end;
  nNetFL := Query3.Fieldbyname('NETPRC').Asstring;

  if (cxButtonEdit1.Text <> '') and (not Query3.Eof and Query3.Bof) then
  begin
    QPoTrn.Append;
    QPoTrnPARTNO.Asstring := nPartno;
    QPoTrnPARTDESC.Asstring := Query3.fieldbyname('DESC1').AsString;
    QPoTrnORDQTY.AsFloat := StrToInt(cxComboBox1.Text);
    if QPoinv1COSTFLG.Asstring = '1' then
      QPoTrn.FieldByName('ORDCOST').AsFloat := Query3.FieldByName('STDBUY').AsFloat;

    if QPoinv1COSTFLG.Asstring = '2' then
      QPoTrn.FieldByName('ORDCOST').AsFloat := Query3.FieldByName('PRICE1').AsFloat;

    QPoTrnREDU.AsFloat := StrToInt(cxTextEdit7.Text);

    nBal := FRound((Query3.fieldbyname('COST').AsFloat * StrToInt(cxComboBox1.Text)), 4);
    nRedu := FRound((QPoTrnREDU.AsFloat * nBal) / 100, 4);
    nTot := FRound(nBal - nRedu, 4);
    QPoTrnNETCOST.AsFloat := nTot;
    QPoTrnTOTCOST.AsFloat := nTot;
    QPoTrnNetFl.Asstring := nNetFL;
...
procedure TFPo_inv02.cxButtonEdit1PropertiesButtonClick(Sender: TObject; AButtonIndex: Integer);
begin
  {Application.Createform(TfFindPO,fFindPO);
  If fFindPO.ShowModalPARTS=Mrok Then
  begin
    ...
    cxButtonEdit1.Text := QInvt.Fieldbyname('PARTNO').Asstring;
    cxTextEdit6.Text   := QInvt.Fieldbyname('DESC1').Asstring;
  end;}
end;

procedure TFPo_inv02.cxTextEdit6FocusChanged(Sender: TObject);
begin
  cxButtonEdit1.SetFocus;
end;

procedure TFPo_inv02.QPoTrnNETCOSTValidate(Sender: TField);
begin
 // QPoTrnTOTCOST.Asfloat := QPoTrnORDQTY.Asfloat * QPoTrnNetcost.Asfloat;
end;

procedure TFPo_inv02.QPoTrnTOTCOSTValidate(Sender: TField);
begin
  if QPoTrnORDQTY.AsFloat > 0 then
